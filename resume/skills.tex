\cvsection{Skills}

\begin{cvskills}
  \cvskill
    {语言} % Type
    {\hyperlink{cpp-proj}{C/C++\textsuperscript{[1][5][6]}}, \hyperlink{shell-proj}{Shell\textsuperscript{[2]}}, \hyperlink{python-proj}{Python\textsuperscript{[3]}}} % Skillset
\end{cvskills}

\begin{cventries}
  \cventry
    {} % Empty position
    {熟悉} % Empty Type
    {} % Empty location
    {} % Empty date
    {
      \begin{cvitems} % Description(s) bullet points
        \item {熟悉 Linux C/C++ 开发，熟练使用 gcc/g++, gdb, makefile, cmake, meson}
        \item {熟悉 GPU 渲染管线和 Linux 图形栈}
        \item {熟悉 OpenGL, GLX, EGL, GBM 图形编程接口}
        \item {熟悉 Linux GPU 驱动调试和性能优化, 熟练使用 apitrace, strace}
        \item {熟悉 Linux 内核驱动 DRM, KMS 框架}
        \item {\hyperlink{mesa-mr}{熟悉开源 GPU 驱动 Mesa Gallium/Kopper/Zink 框架\textsuperscript{[4][7]}}}
      \end{cvitems}
    }
  \cventry
    {} % Empty position
    {了解} % Empty Type
    {} % Empty location
    {} % Empty date
    {
      \begin{cvitems} % Description(s) bullet points
        \item {了解 Xorg 实现和 modesetting/DDX 2D 驱动实现}
        \item {了解 OpenGL, Vulkan 应用开发，调试和性能优化}
        \item {了解 GLFW, SDL, ImGUI 基本使用方法}
        \item {了解 Linux perf 原理及使用方法}
        \item {了解 valgrind, perfetto 等使用方法}
        \item {了解 Jenkins CI 的搭建流程及 Groovy 脚本}
        \item {了解芯片 Bring-up 流程}
        \item {\hyperlink{linux-patches}{了解 Linux kernel patch 提交流程\textsuperscript{[8]}}}
      \end{cvitems}
    }
\end{cventries}
